...
Use the following lines to run CALISTA on Bargaje dataset (available in the subfolder EXAMPLES/BARGAJE):
...
```
1. From the popup window select the dataset **1-BARGAJE_6days_NORM_CT_vlaues_data_type_1_format_data_1_clusters_5.csv** in EXAMPLE/BARGAJE
```{r echo=FALSE,eval=TRUE}
writeLines('[1] "**** Please upload normalized data. File formats accepted: .txt , .xlxs , .csv ****"')

writeLines('Data loading...')
```
2. For cell clustering step, here the number of clusters is determined using the eigengap plot. According to the eigengap plot below, we set the number of clusters to **5** (first eigengap) in the console:
```{r echo=FALSE,eval=TRUE}
writeLines('CALISTA_clustering is running...')

writeLines('Optimal number of cluster according to max. eigenvalue: 8 
