...
Analysis of RNA-seq data in Chu et al., [(Single-cell RNA-seq reveals novel regulators of human embryonic stem cell differentiation to definitive endoderm, Genome Biol. 17, 173 (2016)).](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-016-1033-x)

First download the example datasets from the following github [**link**](https://github.com/CABSEL/calista_example_data)

**Please unzip the file “4-CHU__data_type_4_format_data_2_clusters_4.csv.zip” in EXAMPLES/TREUTLEIN/ before running CALISTA**

Use the following lines to run CALISTA on UNZIPPED Treutlein dataset (available in the subfolder EXAMPLES/CHU):
...
```
1. From the popup window select the dataset **4-CHU__data_type_4_format_data_2_clusters_4.csv** in EXAMPLE/CHU
```{r echo=FALSE,eval=TRUE}
writeLines('[1] "**** Please upload normalized data. File formats accepted: .txt , .xlxs , .csv ****"')

writeLines('Data loading...')
```
2. In this case, the number of clusters is determined using the eigengap plot. According to the eigengap plot below, we set the number of clusters to **4** (first eigengap) in the console:
```{r echo=FALSE,eval=TRUE}
writeLines('CALISTA_clustering is running...')

writeLines('Optimal number of cluster according to max. eigenvalue: 5 
